...
for file in os.listdir(inputDir):
    if file.endswith(".jpg") or file.endswith(".jpeg"):
        ...
        if(img1.shape[1] > img1.shape[0]):
            (img1, ratio) = image_resize(img1, width=1200)
            (org_img, ratio) = image_resize(org_img, width=1200)
        else:
            (img1, ratio) = image_resize(img1, height=1200)
            (org_img, ratio) = image_resize(org_img, height=1200)
        ...
        try:
            ...
            warped = make_document(org, [box], ratio=1/ratio)
